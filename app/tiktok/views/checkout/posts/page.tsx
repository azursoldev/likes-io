...
function PostsSelectionContent() {
  ...
  const totalViews = parseQty(qty);
  const selectedCount = selectedPosts.length || (postLink ? 1 : 0);
  const viewsPerPost = selectedCount > 0 ? Math.floor(totalViews / selectedCount) : totalViews;
  ...
  const handlePostSelect = (url: string) => {
    setSelectedPosts(prev => {
      if (prev.includes(url)) {
        ...
        const potentialViewsPerPost = Math.floor(totalViews / newCount);
        
        // Setting a minimum of 100 views per post to be safe, or 50 if desired. 
        // Given 500 is smallest pack, 50 is reasonable.
        if (potentialViewsPerPost < 50 && totalViews >= 50) {
            setSelectionError("Minimum 50 views per post required. Please deselect a post to add another.");
            return prev;
        }
  ...
  const handleContinue = (e: React.FormEvent) => {
    ...
    if (finalLinks.length > 0) {
      ...
      router.push(`/tiktok/views/checkout/final?username=${username}&qty=${qty}&price=${priceValue}&type=${encodeURIComponent(packageType)}&postLink=${encodeURIComponent(finalLinks.join(','))}&email=${encodeURIComponent(email)}`);
    }
  };
                    ...
                    <div className="posts-grid" style={{ 
                      ...
                      {posts.map((post) => (
                        ...
                        >
                          ...
                          {/* Selected Badge - Bottom Center (Views count) */}
                          {selectedPosts.includes(post.url) && (
                            <div style={{
                              position: 'absolute',
                              bottom: '10px',
                    ...
                    <div className="order-summary-left">
                      ...
                      <div className="order-summary-details">
                        ...
                        <span className="order-summary-subtext">
                          {viewsPerPost} views / {selectedCount} post{selectedCount !== 1 ? 's' : ''}
                        </span>
                      </div>
                    </div>
                    <button type="button" className="order-change-btn" onClick={() => router.push(`/tiktok/views/checkout?qty=${qty}&price=${priceValue}&type=${encodeURIComponent(packageType)}&email=${encodeURIComponent(email)}`)}>Change</button>
                  </div>
